...
<template>
    <section>
        <div class="py-5">
            <div class="mb-5">
                <div class="text-2xl">{{ t('auth.span.email') }}</div>
                <div class="text-5xl text-secondary">{{ data.email }}</div>
            </div>

            <div class="mb-5">
                <div class="text-2xl">{{ t('auth.span.last_login') }}</div>
                <div class="text-3xl text-muted">{{ dateFormatted }}</div>
            </div>
        </div>

        <div class="flex justify-between gap-3">
            <Button class="w-full" @click="logOut">{{ t('auth.span.exit') }}</Button>
            <Button class="w-full" @click="logIn" style-type="secondary">{{ t('auth.span.enter_to_game') }}</Button>
        </div>
    </section>
</template>
